...
function New_Items_Parameter_Estimate=FFMLE_M_Method_A_Online_Calibration_Individual(Theta_Estimate,New_Items_Table,Beta_Hat,Dispersion_Matrix,Parameter_Initial,Accuracy_of_Iteration)
...
for j=1:Number_of_New_Items
    ...
    Beta_j=(Beta_Hat(j,:))';                                        % results from the original M-Method A method
    
    for i=1:(length(Examinee_IDs))
        ID=Examinee_IDs(i,1);
        Theta_True_f(i,:)=((Theta_Estimate(ID,:))'+(Response_Pattern(i,1)-Item_Response_Functions_New(Theta_Estimate(ID,:),Beta_j))*Dispersion_Matrix{ID,1}*Beta_j(1:(Number_of_Dimensions-1),1))';
    end
